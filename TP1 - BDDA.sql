...
SELECT p.numero, p.nom, p.prenom, TREAT(VALUE(p) AS enseignant_type).grade FROM Personne p WHERE VALUE(p) IS OF (enseignant_type);
SELECT p.numero, p.nom, p.prenom, TREAT(VALUE(p) AS etudiant_type).numCarteEtudiant FROM Personne p WHERE VALUE(p) IS OF (etudiant_type);
SELECT p.* FROM Personne p WHERE VALUE(p) IS NOT OF (enseignant_type) AND VALUE(p) IS NOT OF(etudiant_type);


-- Bonus

SELECT user FROM dual; -- Utilisateur actuel
SELECT sys_context('userenv','instance_name') FROM dual; -- get SID (ORCLCDB / ORCLPDB)